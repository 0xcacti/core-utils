...
// };
//
// static struct option const longopts[] = {{"logical", no_argument, NULL, 'L'},
...
//
// static void find_dir_entry(struct stat *dot_sb, struct file_name *file_name, size_t
// parent_height) {
//   DIR *dirp = opendir("..");
//   if (dirp == NULL)
//     error(EXIT_FAILURE, errno, _("cannot open directory %s"), quote(nth_parent(parent_height)));
//
//   int fd = dirfd(dirp);
//   if ((0 <= fd ? fchdir(fd) : chdir("..")) < 0)
//     error(EXIT_FAILURE, errno, _("failed to chdir to %s"), quote(nth_parent(parent_height)));
//
//   struct stat parent_sb;
//   if ((0 <= fd ? fstat(fd, &parent_sb) : stat(".", &parent_sb)) < 0)
//     error(EXIT_FAILURE, errno, _("failed to stat %s"), quote(nth_parent(parent_height)));
//
...
//     error(EXIT_FAILURE, errno, _("reading directory %s"), quote(nth_parent(parent_height)));
//   }
//
//   if (!found)
//     error(EXIT_FAILURE, 0, _("couldn't find directory entry in %s with matching i-node"),
...
//
//   struct stat dot_sb;
//   if (stat(".", &dot_sb) < 0) error(EXIT_FAILURE, errno, _("failed to stat %s"), quoteaf("."));
//
//   while (true) {
//     /* If we've reached the root, we're done.  */
//     if (PSAME_INODE(&dot_sb, root_dev_ino)) break;
//
//     find_dir_entry(&dot_sb, file_name, height++);
//   }
//
//   /* See if a leading slash is needed; file_name_prepend adds one.  */
//   if (file_name->start[0] == '\0') file_name_prepend(file_name, "", 0);
// }
//
// /* Return PWD from the environment if it is acceptable for 'pwd -L'
//    output, otherwise NULL.  */
// static char const *logical_getcwd(void) {
//   char const *wd = getenv("PWD");
//
//   /* Textual validation first.  */
//   if (!wd || wd[0] != '/') return NULL;
//   char const *p = wd;
//   while ((p = strstr(p, "/."))) {
//     if (!p[2] || p[2] == '/' || (p[2] == '.' && (!p[3] || p[3] == '/'))) return NULL;
//     p++;
//   }
//
...
//   struct stat st1;
//   struct stat st2;
//   if (stat(wd, &st1) == 0 && stat(".", &st2) == 0 && psame_inode(&st1, &st2)) return wd;
//   return NULL;
// }
//
...
//   }
//
//   if (optind < argc) error(0, 0, _("ignoring non-option arguments"));
//
//   if (logical) {
